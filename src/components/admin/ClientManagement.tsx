...
import { Plus, Edit, Building2, Phone, Mail, Trash2, Shield, ShieldOff } from 'lucide-react';
...
export const ClientManagement = ({ userRole }: ClientManagementProps) => {
  ...
  const handleAddClient = () => {
    if (!newClient.cnpj.trim() || !newClient.companyName.trim()) {
      toast({
        title: "Erro",
        description: "CNPJ e Nome da empresa são obrigatórios",
        variant: "destructive"
      });
      return;
    }
    ...
    if (!validateCNPJ(newClient.cnpj)) {
      toast({
        title: "Erro",
        description: "CNPJ deve ter formato válido",
        variant: "destructive"
      });
      return;
    }
    ...
    if (clients.some(c => c.cnpj === newClient.cnpj)) {
      toast({
        title: "Erro",
  ...
  const toggleClientStatus = (clientId: string) => {
    ...
    toast({
      title: "Sucesso",
      description: `Cliente ${client.isActive ? 'bloqueado' : 'desbloqueado'} com sucesso`
    });
  };

  const handleDeleteClient = (clientId: string) => {
    // TODO: Check if client has open tickets
    const hasOpenTickets = false; // This would be a real check in production
    
    if (hasOpenTickets) {
      toast({
        title: "Erro",
        description: "Não é possível excluir cliente com chamados em aberto",
        variant: "destructive"
      });
      return;
    }
